...
// Categories derived dynamically from actual products — no hardcoded list
export default function ShopGrid({ products }: { products: Product[] }) {
  const display = products.length > 0 ? products : placeholders;

  // Build category list from actual products in the display set
  const categories = ["All", ...Array.from(new Set(display.map(p => p.category))).sort()];
